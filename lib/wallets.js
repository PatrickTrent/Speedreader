...
export function openWalletStore(dataDir, options = {}) {
  ...
  const saveDispute = db.prepare(
    'UPDATE claims SET dispute_amount = ?, dispute_status = ?, credits_disputed = ? WHERE session_id = ?',
  );
  ...
  function pruneClaims() {
    deleteOldClaims.run(cutoff(CLAIM_RETENTION_MS));
  }
  ...
  function grantCredits(input) {
    ...
    return transaction(() => {
      ...
      if (adj) {
        ...
        if (disputeStatus && disputeStatus !== 'won') {
  ...
  function applyRefund({ chargeId, paymentIntentId, amountRefunded, chargeAmount }) {
    return transaction(() => {
      ...
      const target = creditsToRemove(claim.credits, refunded, total);
      const already = Number(claim.credits_refunded || 0);
      const room = Math.max(0, claim.credits - already - Number(claim.credits_disputed || 0));
      const applied = deductFromWallet(claim.wallet_id, Math.min(Math.max(0, target - already), room));
      const creditsRefunded = already + applied;
      saveRefund.run(refunded, creditsRefunded, claim.session_id);
      return {
        found: true,
        pending: false,
        deducted: applied,
  ...
  function applyDispute({ chargeId, paymentIntentId, amount, status }) {
    const won = status === 'won';
    const disputeStatus = won ? 'won' : status === 'lost' ? 'lost' : 'open';
    return transaction(() => {
      const claim = findClaimByRefs(chargeId, paymentIntentId);
      if (!claim) {
        if (!chargeId && !paymentIntentId) return { found: false, pending: false };
      ...
      if (won) {
        const restore = Number(claim.credits_disputed || 0);
        if (restore > 0) bumpBalance.run(restore, iso(now()), claim.wallet_id);
        saveDispute.run(Number(amount || claim.dispute_amount || 0), 'won', 0, claim.session_id);
        return {
          found: true,
          pending: false,
          restored: restore,
      ...
      const target = creditsToRemove(claim.credits, amount, total);
      const already = Number(claim.credits_disputed || 0);
      const room = Math.max(0, claim.credits - Number(claim.credits_refunded || 0) - already);
      const applied = deductFromWallet(claim.wallet_id, Math.min(Math.max(0, target - already), room));
      saveDispute.run(Number(amount || 0), disputeStatus, already + applied, claim.session_id);
      return {
        found: true,
        pending: false,
        deducted: applied,
