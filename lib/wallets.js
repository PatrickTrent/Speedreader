...
export const ORIGIN_SECRET_MIN_LENGTH = 32;

/** One trimmed value, used for startup checks and every later comparison. */
export function normalizeOriginSecret(value) {
...
/** sqlite (default) or firestore. Empty means sqlite. */
export function normalizeStoreKind(value) {
  const kind = String(value ?? 'sqlite').trim().toLowerCase();
  return kind || 'sqlite';
}

/** Cloud Run sets K_SERVICE. Some jobs also set CLOUD_RUN_*. */
export function isCloudRunEnv(env = process.env) {
  if (String(env?.K_SERVICE || '').trim()) return true;
  return Object.keys(env || {}).some((name) => name.startsWith('CLOUD_RUN_'));
}

/**
 * Production must have a configured store, IP_HASH_SECRET, and ORIGIN_SECRET
 * or DIRECT_CLOUDFLARE_ORIGIN=1. SQLite requires DATA_DIR outside the deploy
 * directory. Firestore requires a project id and does not use DATA_DIR.
...
export function productionConfigError({
  ...
  directCloudflareOrigin = process.env.DIRECT_CLOUDFLARE_ORIGIN,
  store = process.env.STORE,
  firestoreProject = process.env.FIRESTORE_PROJECT || process.env.GOOGLE_CLOUD_PROJECT || process.env.GCLOUD_PROJECT,
  env = process.env,
  cloudRun,
  ...
  const kind = normalizeStoreKind(store);
  if (kind !== 'sqlite' && kind !== 'firestore') {
    return `STORE=${kind} is not sqlite or firestore`;
  }
  const onCloudRun = cloudRun !== undefined ? Boolean(cloudRun) : isCloudRunEnv(env);
  if (kind !== 'firestore' && onCloudRun) {
    return 'SQLite cannot run on Cloud Run; set STORE=firestore';
  }
  if (kind === 'firestore') {
    if (!String(firestoreProject || '').trim()) {
      return 'Firestore project id is unset (set FIRESTORE_PROJECT; Cloud Run does not set GOOGLE_CLOUD_PROJECT)';
    }
  } else {
  ...
  const secret = normalizeOriginSecret(originSecret);
  if (secret && secret.length < ORIGIN_SECRET_MIN_LENGTH) {
    return 'ORIGIN_SECRET is shorter than 32 characters';
  }
  if (!secret && String(directCloudflareOrigin || '') !== '1') {
    return 'ORIGIN_SECRET is unset and DIRECT_CLOUDFLARE_ORIGIN is not 1';
  }
  return null;
}
...
export function openWalletStore(dataDir, options = {}) {
  ...
  const deleteIdlePaid = db.prepare(
    `DELETE FROM wallets
     WHERE has_payment = 1 AND balance = 0 AND last_activity_at < ?
       AND id NOT IN (SELECT wallet_id FROM claims WHERE created_at >= ?)`,
  );
  ...
  function prunePaidWallets() {
    deleteIdlePaid.run(cutoff(PAID_WALLET_IDLE_MS), cutoff(FREE_WALLET_IDLE_MS));
  }
  ...
  function applyRefund({ chargeId, paymentIntentId, amountRefunded, chargeAmount }) {
    return transaction(() => {
      ...
      // Credits an open dispute is still holding. After won or warning_closed this is 0.
