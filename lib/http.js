...
export function createApp(options = {}) {
  ...
  app.use((req, res, next) => {
    ...
    if (WALLET_VARY_PATHS.has(req.path)) res.set('Vary', 'X-Wallet-Id');
...
function main() {
  ...
  if (!process.env.ORIGIN_SECRET) {
    console.error('DIRECT_CLOUDFLARE_ORIGIN=1 and ORIGIN_SECRET is unset. CF-Connecting-IP is trusted only when the TCP peer is in a Cloudflare range. A request from 169.254.0.0/16 or a Google load balancer range returns 503 origin_misconfigured on /api/wallet, /api/checkout, /api/claim, and /api/summarize, and those routes are not placed in a shared rate-limit bucket.');
  }
