...
import { openFirestoreStore } from './firestore-store.js';
...
function main() {
  ...
  if (configError) {
    console.error(`REFUSING TO START: ${configError}. For SQLite, DATA_DIR must be a persistent volume outside the deploy directory. For Cloud Run set STORE=firestore (GOOGLE_CLOUD_PROJECT is set on the service; FIRESTORE_DATABASE is optional). IP_HASH_SECRET must be set, and production must set ORIGIN_SECRET or DIRECT_CLOUDFLARE_ORIGIN=1.`);
    process.exit(1);
  }
