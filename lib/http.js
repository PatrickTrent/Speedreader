...
/**
 * CF-Connecting-IP is used only when the TCP peer is Cloudflare, or when
 * ORIGIN_SECRET matches X-Origin-Secret. An invalid header falls back to the
 * socket address. X-Forwarded-For is never read.
 */
export function clientAddress(req, trustCloudflare, originSecret = '') {
  const rawSocket = req.socket?.remoteAddress || '';
  const socketGroup = groupIp(rawSocket) || rawSocket || 'missing-socket';
  if (!trustCloudflare) return socketGroup;
  const trustedPeer = isCloudflareAddress(rawSocket) || originSecretMatches(req, originSecret);
  if (!trustedPeer) return socketGroup;
  const header = headerValue(req, 'cf-connecting-ip').split(',')[0].trim();
  return (header && groupIp(header)) || socketGroup;
}
...
export function createApp(options = {}) {
  ...
  const limiters = {
    config: createRateLimiter(limits.config || { windowMs: 60_000, max: 30 }),
  ...
  function ipOf(req) {
    return clientAddress(req, trustCloudflare, originSecret);
  }
      ...
      if (type === 'charge.refunded' || type === 'charge.dispute.created' || type === 'charge.dispute.closed') {
        ...
        try {
          ...
          if (result.deducted > 0 || result.restored > 0) {
            console.error(`credits adjusted wallet=${result.walletId} deducted=${result.deducted || 0} restored=${result.restored || 0} balance=${result.balance} event=${type}`);
