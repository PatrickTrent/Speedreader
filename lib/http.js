...
import { openFirestoreStore, resolveFirestoreProject } from './firestore-store.js';
...
function secretLiterals(env) {
  ...
  for (const name of SECRET_ENV_NAMES) {
    ...
    if (raw.length >= MIN_SECRET_LITERAL) found.push(raw);
    if (trimmed.length >= MIN_SECRET_LITERAL && trimmed !== raw) found.push(trimmed);
  }
...
export function safeErrorMessage(err, env = process.env) {
  const raw = typeof err === 'string'
    ? err
    : (err && typeof err === 'object' && err.message ? String(err.message) : 'unknown error');
  let text = raw;
  for (const secret of secretLiterals(env)) {
    text = text.split(secret).join('[redacted]');
  }
  return text
    .replace(/Bearer\s+\S+/gi, 'Bearer [redacted]')
    .replace(/\b(sk|rk)_(?:live|test)_[A-Za-z0-9]+\b/g, '$1_[redacted]')
    .replace(/\bwhsec_[A-Za-z0-9]+\b/g, 'whsec_[redacted]')
    .replace(/\bAIza[0-9A-Za-z_-]{10,}(?![0-9A-Za-z_-])/g, '[redacted]')
    .replace(/-----BEGIN [A-Z ]*PRIVATE KEY-----[\s\S]*?-----END [A-Z ]*PRIVATE KEY-----/g, '[redacted key]')
    .slice(0, 300);
}

export function logUnhandledRejection(err) {
...
function stripeWebhookRequest(req) {
  const q = req.url.indexOf('?');
  const pathOnly = q === -1 ? req.url : req.url.slice(0, q);
  return req.method === 'POST' && pathOnly === '/api/stripe-webhook';
}
...
/**
 * Cloud Run (169.254.0.0/16) and Google load-balancer peers cannot share a
 * rate-limit bucket. Without ORIGIN_SECRET those credit routes are refused.
 */
export function googlePeerWithoutOriginSecret(peer, originSecret = '') {
  if (String(originSecret || '').trim()) return false;
  return isGoogleFrontendAddress(peer);
}
...
let originMisconfiguredWarned = false;

function warnOriginMisconfiguredOnce(peer) {
  if (originMisconfiguredWarned) return;
  originMisconfiguredWarned = true;
  console.error(`ORIGIN MISCONFIGURED peer ${peer}. A request arrived from 169.254.0.0/16 or a Google load balancer range and ORIGIN_SECRET is unset. /api/wallet, /api/checkout, /api/claim, and /api/summarize return 503 origin_misconfigured and are not placed in a shared rate-limit bucket. Set ORIGIN_SECRET, or set DIRECT_CLOUDFLARE_ORIGIN=1 only when Cloudflare connects to this process directly. See DEPLOY.md.`);
}

const WALLET_VARY_PATHS = new Set(['/api/wallet', '/api/checkout', '/api/claim', '/api/summarize']);
...
const ORIGIN_GUARDED_ROUTES = new Set(['wallet', 'checkout', 'claim', 'summarize']);
...
export function createApp(options = {}) {
  ...
  const originSecret = normalizeOriginSecret(
    options.originSecret !== undefined ? options.originSecret : process.env.ORIGIN_SECRET,
  );
  ...
  let store = null;
  const kind = normalizeStoreKind(options.storeKind !== undefined ? options.storeKind : process.env.STORE);
  function noteEphemeral(opened) {
  ...
  if (options.failDatabase) {
    ...
    const opened = openFirestoreStore({
      projectId: options.firestoreProject,
      databaseId: options.firestoreDatabase,
      client: options.firestoreClient,
      now,
    ...
    if (!opened.ok) {
      console.error('WALLET DATABASE FAILED. Payments and summaries are disabled. Check STORE=firestore, FIRESTORE_PROJECT or GOOGLE_CLOUD_PROJECT, and the Cloud Run service account.');
      console.error(opened.error?.message || 'unknown database error');
    } else {
      store = opened;
      noteEphemeral(opened);
    }
  } else if (kind !== 'sqlite') {
    console.error(`WALLET DATABASE FAILED. Payments and summaries are disabled. STORE=${kind} is not sqlite or firestore.`);
  } else {
  ...
  function walletFromRequest(req) {
    const header = walletFrom(headerValue(req, 'x-wallet-id'));
    if (header) return header;
    const bodyId = req.body && typeof req.body.walletId === 'string' ? req.body.walletId : '';
    return walletFrom(bodyId);
  }

  function limited(name, req, res) {
    const rawSocket = req.socket?.remoteAddress || '';
    if (ORIGIN_GUARDED_ROUTES.has(name) && googlePeerWithoutOriginSecret(rawSocket, originSecret)) {
      warnOriginMisconfiguredOnce(rawSocket);
      res.status(503).json({ error: 'origin_misconfigured' });
      return true;
    }
  ...
  async function fulfill(sessionId) {
    ...
    return await store.grantCredits(paid);
  }
  ...
  function catchRequest(handler) {
    return (req, res, next) => {
      Promise.resolve(handler(req, res, next)).catch((err) => {
        ...
        console.error('request failed');
        console.error(safeErrorMessage(err));
        const status = err?.public && Number(err.status) >= 400 && Number(err.status) < 600 ? Number(err.status) : 500;
  ...
  app.use((req, res, next) => {
    ...
    if (collapsed !== pathOnly) req.url = collapsed + query;
    if (originSecret && !stripeWebhookRequest(req) && !originSecretMatches(req, originSecret)) {
      res.set('Cache-Control', 'no-store');
      res.status(403).json({ error: 'origin_forbidden' });
      return;
    }
    const apiPath = normalizedApiPath(req.path);
    if (apiPath !== '/api' && !apiPath.startsWith('/api/')) return next();
    res.set('Cache-Control', 'no-store');
    if (WALLET_VARY_PATHS.has(apiPath)) res.set('Vary', 'X-Wallet-Id');
    const setHeader = res.setHeader;
    res.setHeader = function skipApiEtag(name, value) {
      if (String(name).toLowerCase() === 'etag') return this;
  ...
  api.get('/config', (req, res) => {
    ...
    const rawSocket = req.socket?.remoteAddress || '';
    if (googlePeerWithoutOriginSecret(rawSocket, originSecret)) {
      warnOriginMisconfiguredOnce(rawSocket);
      return res.json({ payments: false, summaries: false });
    }
  ...
  api.get('/wallet', catchRequest(async (req, res) => {
    if (limited('wallet', req, res)) return undefined;
    const walletId = walletFromRequest(req);
    ...
    try {
      const result = await store.preview(walletId, ipOf(req));
  ...
  api.post('/checkout', catchRequest(async (req, res) => {
  ...
  api.post('/claim', catchRequest(async (req, res) => {
  ...
  api.post('/summarize', catchRequest(async (req, res) => {
    ...
    let reserved = false;
    try {
      const reservation = await store.beginSummarize(walletId, ipOf(req));
      ...
      return res.json({ summary: summary.toUpperCase(), balance: await store.balanceOf(walletId) });
    } catch (err) {
      let balance;
      if (reserved) {
        try {
          balance = (await store.refundCredit(walletId)).balance;
        } catch {
          balance = undefined;
        }
      }
...
`);
  ...
  app.storeReady = Boolean(store);
  app.whenReady = async () => {
    if (typeof store?.probe === 'function') await store.probe();
  };
...
async function main() {
  ...
  const firestoreProject = await resolveFirestoreProject();
  const configError = productionConfigError(firestoreProject ? { firestoreProject } : {});
  if (configError) {
    console.error(`REFUSING TO START: ${configError}. For SQLite, DATA_DIR must be a persistent volume outside the deploy directory, and SQLite is refused on Cloud Run. For Cloud Run set STORE=firestore and FIRESTORE_PROJECT (Cloud Run does not set GOOGLE_CLOUD_PROJECT). IP_HASH_SECRET must be set, and production must set ORIGIN_SECRET or DIRECT_CLOUDFLARE_ORIGIN=1.`);
    process.exit(1);
  }
  const originSecret = normalizeOriginSecret(process.env.ORIGIN_SECRET);
  if (!originSecret) {
    const direct = String(process.env.DIRECT_CLOUDFLARE_ORIGIN || '') === '1';
    const where = direct
      ? 'ORIGIN_SECRET is unset. DIRECT_CLOUDFLARE_ORIGIN=1, so Cloudflare is expected to connect to this process directly.'
      : 'ORIGIN_SECRET is unset.';
    console.error(`${where} CF-Connecting-IP is trusted only when the TCP peer is in a Cloudflare range. A request from 169.254.0.0/16 or a Google load balancer range returns 503 origin_misconfigured on /api/wallet, /api/checkout, /api/claim, and /api/summarize, /api/config reports payments and summaries off, and those credit routes are not placed in a shared rate-limit bucket.`);
  }
  if (process.env.TRUST_FORWARDED_FROM === 'google' && !originSecret) {
  ...
  const app = createApp(firestoreProject ? { firestoreProject } : {});
  if (normalizeStoreKind(process.env.STORE) === 'firestore' && !app.storeReady) {
    console.error('REFUSING TO START: Firestore store did not open. Check FIRESTORE_PROJECT and the runtime service account.');
    try { app.closeStore(); } catch { /* already closed */ }
    process.exit(1);
  }
  try {
    await app.whenReady();
  } catch (err) {
    console.error('REFUSING TO START: Firestore startup read failed. Check FIRESTORE_PROJECT, FIRESTORE_DATABASE, and roles/datastore.user on the runtime service account.');
    console.error(safeErrorMessage(err));
    try { app.closeStore(); } catch { /* already closed */ }
    process.exit(1);
  }
